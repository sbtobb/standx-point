...
## Clippy-Driven Refactors (2026-02-05)

Fixed clippy errors in `standx-point-adapter` by:
1. Using `strip_suffix` instead of manual slicing in `persistent_key.rs`
2. Collapsing nested `if let` chains in `persistent_key.rs` and `ws/client.rs`
3. Removing redundant closure in `solana_wallet.rs` (changed `map_err(|e| StandxError::Serialization(e))` to `map_err(StandxError::Serialization)`)
4. Removing `clone()` on `Copy` type `Chain` in `wallet.rs`
5. Changing `&Vec<Decimal>` parameter to `&[Decimal]` in `types/models.rs`

All changes are minimal and focused on addressing clippy warnings while maintaining functionality.
